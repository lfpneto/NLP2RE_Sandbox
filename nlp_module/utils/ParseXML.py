...
def parse_xml_to_dataframe(xml_file_path, namespace=None):
    ...
    def extract_data(element, path=''):
        tag_without_namespace = element.tag.replace(namespace.get('ns', '') + '}', '') if namespace else element.tag
        data = {
        ...
        for child in element:
            child_path = path + '/' + tag_without_namespace.replace('{', '') if path else tag_without_namespace.replace('{', '')
...
def replace_multiple_spaces(df):
    ...
    df['text'] = df['text'].apply(lambda x: re.sub(r'\s+', ' ', x) if isinstance(x, str) else x)
